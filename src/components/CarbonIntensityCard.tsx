...
const CarbonIntensityCard = ({ intensityData }: Props) => {
  const arrowColor = intensityData.intensity.index === 'low' ? 'text-green-600' : 'text-red-600';
  const arrowDirection =intensityData.intensity.index === 'low' ? 'rotate-180' : ''; // Rotate the arrow 180 degrees if index is 'low'
  ...
  return (
    <div className="max-w-xs mx-auto bg-white rounded-2xl shadow-md p-4 flex flex-col items-center">
      ...
      {intensityData.intensity.index !== 'low' ? (
